...
FOR i = 1 TO k - 1
	...
	IF RECCOUNT('cur_unic')=0
		IF Arr[i,5] = 1
			INSERT INTO tmp(dt,ci,np,kolvo,kolvo_real,invtp) VALUES (cur_date,Arr[i,1],Arr[i,2],Arr[i,3],'0','Рабочие наряды')
		ELSE
			INSERT INTO tmp(dt,ci,np,kolvo,kolvo_real,invtp) VALUES (cur_date,Arr[i,1],Arr[i,2],Arr[i,3],'0','Бригадные ведомости')
		ENDIF  
	ELSE
		SELECT 'cur_unic'
		RELEASE Arr1
...
,Arr[i,3],Arr1[1],'Рабочие наряды')
			ELSE
				INSERT INTO tmp(dt,ci,np,kolvo,kolvo_real,invtp) VALUES (cur_date,Arr[i,1],Arr[i,2];
,Arr[i,3],Arr1[1],'Бригадные ведомости')
 			ENDIF
		ENDIF
	ENDIF 
ENDFOR
