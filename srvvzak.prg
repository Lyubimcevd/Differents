...
str_query = "select top 200 convert(varchar(4),np) as np,kolvo,kolvo_real,convert(char(10),dt,104) as dt2 from bpd.registr_documents where doc_type = ";
	+STR(ctype)+" order by dt desc"
...
SCAN WHILE ISNULL(kolvo_real)
	...
	ELSE
		SELECT 'tmp_cur'
		cur_kolvo_real = k
		str_query = "UPDATE bpd.registr_documents SET kolvo_real ="+STR(cur_kolvo_real)+" WHERE dt=convert(date,'"+cur_date+"') AND NP="+cur_np;
			+" and doc_type = "+STR(ctype)
