...
IF RECCOUNT('cur_unic')#0
	...
	FOR i = 1 TO ALEN(Arr,1)
		...
		str_query = "select COUNT(*) FROM bpd.invnez WHERE grup<700 and ci = "+cur_ci+" AND np = "+cur_np
		DO CASE
			CASE Arr[i,4] = 5
		...
		ELSE 
			...
			str_query = "UPDATE bpd.registr_documents SET kolvo_real =";
							+ ALLTRIM(STR(Arr1[1,1]))+" ,WHERE CI=";
							+cur_ci+" AND NP="+cur_np+" AND kolvo=";
							+cur_kolvo+" and doc_type = "+ALLTRIM(STR(ctype))+" and invtp=";
							+ALLTRIM(STR(Arr[i,4]))
	...
	str_query = "select convert(smallint,Ci) as ci,convert(smallint,np) as np,COUNT(*) FROM;
	 bpd.invnez WHERE grup<700 and bs=56 group by ci,np"
	b = SQLEXEC(con_bd,str_query,'cur_unic')
	IF RECCOUNT('cur_unic')#0
		SELECT 'cur_unic'
		RELEASE Arr
		...
		FOR i = 1 TO ALEN(Arr,1)
			...
			str_query = "select kolvo from bpd.registr_documents where ci = "+cur_ci+" and np = ";
						+cur_np+" and doc_type = "+ALLTRIM(STR(ctype))+" and invtp=5"
			b = SQLEXEC(con_bd,str_query,'cur_unic')
			IF RECCOUNT('cur_unic') = 0
	...
	ENDIF
	str_query = "select convert(smallint,Ci) as ci,convert(smallint,np) as np,COUNT(*) FROM;
	 bpd.invnez WHERE grup<700 and bs=57 group by ci,np"
	b = SQLEXEC(con_bd,str_query,'cur_unic')
	IF RECCOUNT('cur_unic')#0
		SELECT 'cur_unic'
		RELEASE Arr
		...
		FOR i = 1 TO ALEN(Arr,1)
			...
			cur_np = ALLTRIM(STR(Arr[i,2]))
			str_query = "select kolvo from bpd.registr_documents where ci = "+cur_ci+" and np = ";
						+cur_np+" and doc_type = "+ALLTRIM(STR(ctype))+" and invtp=6"
			b = SQLEXEC(con_bd,str_query,'cur_unic')
			IF RECCOUNT('cur_unic') = 0
	...
	ENDIF
ELSE
	MESSAGEBOX("Нет данных для сравнения",0,cPrName)
ENDIF 
