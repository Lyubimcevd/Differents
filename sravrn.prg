...
str_query = "select convert(varchar(4),ci) as ci,convert(varchar(4),np) as np,kolvo,convert(varchar(1),invtp) as invtp,";
			+"convert(char(10),dt,104) as dt2 from bpd.registr_documents where doc_type = "+STR(ctype)+" and is_check = 0"
...
b = SQLEXEC(con_bd,str_query,'cur_rn_oper')
SELECT 'cur_rn_oper'
SCAN 
	cur_ci = ALLTRIM(ci)
	cur_np = ALLTRIM(np)
	...
	SELECT * FROM cur_rn WHERE np = cur_np AND ci = cur_ci AND invtp = '1' INTO CURSOR tmp_cur
	IF RECCOUNT('tmp_cur') = 0
...
str_query = "select convert(varchar(4),convert(smallint,Ce)) as ci,convert(varchar(4),convert(smallint,np)) as np,COUNT(*) as k FROM bpd.brigv";
			+" WHERE grup<700 group by ce,np"
b = SQLEXEC(con_bd,str_query,'cur_brigv_oper')
SELECT 'cur_brigv_oper'
SCAN 
	cur_ci = ALLTRIM(ci)
	cur_np = ALLTRIM(np)
	cur_kolvo_real = k
	...
	SELECT 'tmp_cur'
	cur_date = STUFF(STUFF(dvv,3,0,'.'),6,0,'.')
	SELECT * FROM cur_rn WHERE np = cur_np AND ci = cur_ci AND invtp = '2' INTO CURSOR tmp_cur
	IF RECCOUNT('tmp_cur') = 0
