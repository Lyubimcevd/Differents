...
str_query = "select top 200 convert(varchar(4),np) as np,kolvo,kolvo_real,convert(char(10),dt,104) as dt2 from";
			+" bpd.registr_documents where doc_type = "+STR(ctype)+" and invtp = 2 order by dt desc"
b = SQLEXEC(con_bd,str_query,'cur_tmpk')
SELECT 'cur_tmpk'
SCAN WHILE ISNULL(kolvo_real)
	cur_date = dt2
	cur_np = ALLTRIM(np)
	cur_kolvo = kolvo
	str_query = "select COUNT(*) as k FROM bpd.m56k WHERE grup<700 AND np = "+cur_np+" group by np"
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	IF RECCOUNT('tmp_cur')=0
		INSERT INTO tmp(dt,np,kolvo,kolvo_real) VALUES (cur_date,cur_np,cur_kolvo,0)
	ELSE
		...
		cur_kolvo_real = k
		str_query = "UPDATE bpd.registr_documents SET kolvo_real ="+STR(cur_kolvo_real)+" WHERE dt=convert(date,'"+cur_date+"') and np=";
					+cur_np+" and doc_type = "+STR(ctype)+" and invtp = 2"
		b = SQLEXEC(con_bd,str_query)
		IF cur_kolvo#cur_kolvo_real
			INSERT INTO tmp(dt,np,kolvo,kolvo_real) VALUES (cur_date,cur_np,cur_kolvo,cur_kolvo_real)
		ENDIF		
...
str_query = "select convert(varchar(4),convert(smallint,np)) as np,COUNT(*) as k FROM bpd.m56k WHERE grup<700 group by np"
b = SQLEXEC(con_bd,str_query,'cur_tmpk')
SELECT 'cur_tmpk'
SCAN
	cur_np = ALLTRIM(np) 
...
str_query = "select top 200 convert(varchar(4),np) as np,kolvo,kolvo_real,convert(char(10),dt,104) as dt2 from";
			+" bpd.registr_documents where doc_type = "+STR(ctype)+" and invtp = 3 order by dt desc"
b = SQLEXEC(con_bd,str_query,'cur_tmpk')
SELECT 'cur_tmpk'
SCAN WHILE ISNULL(kolvo_real)
	cur_date = dt2
	cur_np = ALLTRIM(np)
	cur_kolvo = kolvo
	str_query = "select COUNT(*) as k FROM bpd.m57k WHERE grup<700 AND np = "+cur_np+" group by np"
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	IF RECCOUNT('tmp_cur')=0
		INSERT INTO tmp(dt,np,kolvo,kolvo_real) VALUES (cur_date,cur_np,cur_kolvo,0)
	ELSE
		...
		cur_kolvo_real = k
		str_query = "UPDATE bpd.registr_documents SET kolvo_real ="+STR(cur_kolvo_real)+" WHERE dt=convert(date,'"+cur_date+"') and np=";
					+cur_np+" and doc_type = "+STR(ctype)+" and invtp = 3"
		b = SQLEXEC(con_bd,str_query)
		IF cur_kolvo#cur_kolvo_real
			INSERT INTO tmp(dt,np,kolvo,kolvo_real) VALUES (cur_date,cur_np,cur_kolvo,cur_kolvo_real)
		ENDIF		
...
str_query = "select convert(varchar(4),convert(smallint,np)) as np,COUNT(*) as k FROM bpd.m57k WHERE grup<700 group by np"
b = SQLEXEC(con_bd,str_query,'cur_tmpk')
SELECT 'cur_tmpk'
SCAN
	cur_np = ALLTRIM(np) 
...
SCAN WHILE ISNULL(kolvo_real)
	...
	cur_np = ALLTRIM(np)
	cur_kolvo = kolvo
	str_query = "select COUNT(*) as k FROM bpd.razzag WHERE grup<700 AND np = "+cur_np+" group by np"
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	IF RECCOUNT('tmp_cur')=0
		INSERT INTO tmp(dt,np,kolvo,kolvo_real) VALUES (cur_date,cur_np,cur_kolvo,0)
	ELSE
