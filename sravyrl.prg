...
*нормализация таблицы( удаление одинаковых сток и их суммирование )
...
FOR i = 1 TO ALEN(Arr,1)
	...
	IF (Arr[i,3] # Arr2[1])
		...
		str_query = "INSERT INTO bpd.registr_documents(dt,ci,np,kolvo,doc_type) ";
 					+VALUES(convert(date,'"+cur_date+"'),"+cur_ci+","+cur_np+",";
 					+ALLTRIM(STR(Arr2[1]))+",120)"
 		b = SQLEXEC(con_bd,str_query,'cur_unic')
 	ENDIF
ENDFOR
...
FOR i = 1 TO k - 1
	...
	kolvo_real_ch = 0
	IF cur_ci == '2'
		str_query = "select id_shtrih_kod from bpd.soprovod_yarl_header where grup<700 and ci=002";
					+" and np = "+PADL(cur_np,3,'0') +" and dvv = '"+CHRTRAN(cur_date,'.','')+"'"
		b = SQLEXEC(con_bd,str_query,'cur_unic')
