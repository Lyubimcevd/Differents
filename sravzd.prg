...
?REPL('-',(89))
?CPCONVERT(1251,866,':    Дата    : Пачка : Номер уведомления : Количество зарегист. : Количество подготовл. :')
?REPL('-',(89))
?

CREATE CURSOR tmp(dt C(10),ci V(4),np V(4),kolvo N,kolvo_real N)
str_query = "select convert(varchar(4),ci) as ci,convert(varchar(4),np) as np,kolvo,convert(char(10),dt,104) as dt2 from bpd.registr_documents where";
	+" doc_type = "+STR(ctype)+" and is_check = 0"
...
b = SQLEXEC(con_bd,str_query,'cur_zd_oper')
SELECT 'cur_zd_oper'
SCAN
	cur_ci = ALLTRIM(cex)
	cur_np = ALLTRIM(np)
	...
	SELECT * FROM cur_zd WHERE np = cur_np INTO CURSOR tmp_cur
	IF RECCOUNT('tmp_cur') = 0
