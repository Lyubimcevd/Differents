...
str_query = "select convert(varchar(4),ci) as ci,convert(varchar(4),np),kolvo,kolvo_real,convert(varchar(1),invtp) as invtp,";
			+"convert(char(10),dt,104) as dt2 from bpd.registr_documents where doc_type = "+STR(ctype)
...
SCAN 
	...
	str_query = "select kolvo from bpd.registr_documents where ci = "+cur_ci+" and np = "+cur_np+" and doc_type = "+STR(ctype)+" and invtp=5"
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	IF RECCOUNT('tmp_cur') = 0
...
SCAN 
	...
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	cur_date = STUFF(STUFF(dvv,3,0,'.'),6,0,'.')
	str_query = "select kolvo from bpd.registr_documents where ci = "+cur_ci+" and np = "+cur_np+" and doc_type = "+STR(ctype)+" and invtp=6"
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	IF RECCOUNT('tmp_cur') = 0
