...
?REPL('-',(82))
?CPCONVERT(1251,866,': Цех : Пачка : Количество зарегист. : Количество подготовл. :        Тип        :')
?REPL('-',(82))
?
...
FOR i = 1 TO k - 1
	...
	IF RECCOUNT('cur_unic')=0
		IF Arr[i,5] = 1
			INSERT INTO tmp(ci,np,kolvo,kolvo_real,invtp) VALUES (Arr[i,1],Arr[i,2],cur_kolvo,'0','Рабочие наряды')
		ELSE
			INSERT INTO tmp(ci,np,kolvo,kolvo_real,invtp) VALUES (Arr[i,1],Arr[i,2],cur_kolvo,'0','Бригадные ведомости')
		ENDIF  
	ELSE
		SELECT 'cur_unic'
		RELEASE Arr1
 ...
 ALLTRIM(STR(Arr1[1])),'Рабочие наряды')
			ELSE
				INSERT INTO tmp(ci,np,kolvo,kolvo_real,invtp) VALUES (Arr[i,1],Arr[i,2],cur_kolvo,;
 ALLTRIM(STR(Arr1[1])),'Бригадные ведомости')
 				ENDIF
		ENDIF
	ENDIF 
ENDFOR
...
IF RECCOUNT('cur_unic')#0
	...
	FOR i = 1 TO ALEN(Arr,1)
		...
		b = SQLEXEC(con_bd,str_query,'cur_unic')
		IF RECCOUNT('cur_unic') = 0
			INSERT INTO tmp(ci,np,kolvo,kolvo_real,invtp) VALUES (Arr[i,1],Arr[i,2],'0',ALLTRIM(STR(Arr[i,3])),'Рабочие наряды')
		ENDIF
	ENDFOR
ENDIF 
str_query = "select convert(smallint,Ce) as ce,convert(smallint,np) as np,COUNT(*) FROM;
...
IF RECCOUNT('cur_unic')#0
	...
	FOR i = 1 TO ALEN(Arr,1)
		...
		b = SQLEXEC(con_bd,str_query,'cur_unic')
		IF RECCOUNT('cur_unic') = 0
			INSERT INTO tmp(ci,np,kolvo,kolvo_real,invtp) VALUES (Arr[i,1],Arr[i,2],'0',ALLTRIM(STR(Arr[i,3])),'Бригадные ведомости')
		ENDIF
	ENDFOR
ENDIF 
IF RECCOUNT('tmp')#0
