...
CREATE CURSOR tmp(dt C(10),ci V(4),np V(4),kolvo N,kolvo_real N,invtp V(12))
...
SCAN WHILE ISNULL(kolvo_real)
	...
	ELSE 
		...
		str_query = "UPDATE bpd.registr_documents SET kolvo_real ="+STR(cur_kolvo_real)+" WHERE CI="+cur_ci+" AND NP="+cur_np+" AND kolvo=";
					+STR(cur_kolvo)+" and doc_type = "+STR(ctype)
...
SCAN
	...
	str_query = "select kolvo from bpd.registr_documents where ci = "+cur_ci+" and np = "+cur_np+" and doc_type = "+STR(ctype)+" and invtp=1"
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	IF RECCOUNT('tmp_cur') = 0
...
SCAN
	...
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	cur_date = STUFF(STUFF(dvv,3,0,'.'),6,0,'.')
	str_query = "select kolvo from bpd.registr_documents where ci = "+cur_ci+" and np = "+cur_np+" and doc_type = "+STR(ctype)+" and invtp=2"
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	IF RECCOUNT('tmp_cur') = 0
...
SCAN
	...
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	cur_date = STUFF(STUFF(dvv,3,0,'.'),6,0,'.')
	str_query = "select kolvo from bpd.registr_documents where ci = "+cur_ci+" and np = "+cur_np+" and doc_type = "+STR(ctype)+" and invtp=3"
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	IF RECCOUNT('tmp_cur') = 0
...
SCAN
	...
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	cur_date = STUFF(STUFF(dvv,3,0,'.'),6,0,'.')
	str_query = "select kolvo from bpd.registr_documents where ci = "+cur_ci+" and np = "+cur_np+" and doc_type = "+STR(ctype)+" and invtp=4"
	b = SQLEXEC(con_bd,str_query,'tmp_cur')
	IF RECCOUNT('tmp_cur') = 0
