...
?REPL('-',(67))
?CPCONVERT(1251,866,':    Дата    : Цех : Пачка : Количество зарегист. : Количество подготовл. :')
?REPL('-',(67))
?
...
b = SQLEXEC(con_bd,str_query,'cur_cou')
SELECT 'cur_cou'
SCAN
 	cur_date = dt2
 	cur_ci = ALLTRIM(ci)
 	cur_np = ALLTRIM(np)
 	cur_kolvo = kolvo
 	str_query = "select COUNT(*) as k FROM bpd.cou WHERE grup<700 and grup != 000 AND ci = "+cur_ci+" AND np = "+cur_np+" group by ci,np "
	...
	ELSE
		...
		str_query = "UPDATE bpd.registr_documents SET kolvo_real ="+STR(cur_kolvo_real)+",is_check = 1 WHERE CI="+cur_ci+" AND NP="+cur_np;
			+" and doc_type = "+STR(ctype)+" and is_check = 0"
...
str_query = "select convert(varchar(4),convert(smallint, ci)) as ci,convert(varchar(4),convert(smallint,np)) as np,COUNT(*) as k FROM bpd.cou";
			+" WHERE grup<700 and grup != 000 group by ci,np"
b = SQLEXEC(con_bd,str_query,'cur_cou_oper')
SELECT 'cur_cou_oper'
SCAN
	cur_ci = ALLTRIM(ci)
	cur_np = ALLTRIM(np) 
	...
	SELECT * FROM cur_cou WHERE np = cur_np AND ci = cur_ci INTO CURSOR tmp_cur
	IF RECCOUNT('tmp_cur') = 0
